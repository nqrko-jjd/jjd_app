...
function EquipeInner() {
  ...
  const [tab, setTab] = useState<'annuaire' | 'disponibilites'>('annuaire');
  ...
  const onAvail = tab === 'disponibilites';
  const windowTo = useMemo(() => addDaysStr(day, 21), [day]);
  const { data: evData, reload: reloadEvents } = useApi<{ items: PlanningEv[] }>(onAvail ? `/api/planning?from=${day}&to=${windowTo}` : null);
  const { data: absData, reload: reloadAbsences } = useApi<{ items: PlanAbsence[] }>(onAvail ? `/api/absences?from=${day}&to=${windowTo}` : null);
  const { data: wsData } = useApi<{ items: WsRow[] }>(onAvail ? `/api/worksites?status=${WORKSITE_STATUS_OPEN.join(',')}` : null);
  const { data: vehData } = useApi<{ items: VehicleRow[] }>(onAvail ? '/api/vehicles' : null);
  const { data: equipData } = useApi<{ items: EquipRow[] }>(onAvail ? '/api/equipment' : null);
  ...
  const activePeople = useMemo(() => (data?.items ?? []).filter((p) => p.active), [data]);
  const availInfo = useMemo(() => {
    ...
    return map;
  }, [activePeople, events, absences, day]);
  ...
  const availRows = useMemo(
    () => activePeople.filter((p) => statusFilter === 'all' || availInfo.get(p.id)?.status === statusFilter),
    [activePeople, availInfo, statusFilter],
  );
  ...
  return (
    <>
        ...
        action={
          tab === 'annuaire' ? (
            ...
            <div className="row">
              ...
              <button className="btn primary" onClick={() => setAssignmentModal({ prefill: { date: day } })}><CalendarPlus size={15} strokeWidth={2} /> Nouvelle affectation</button>
            </div>
          )
        }
      />
      ...
      <div className="seg" style={{ marginBottom: '1.1rem' }}>
        <button type="button" className={tab === 'annuaire' ? 'on' : ''} onClick={() => setTab('annuaire')}>Annuaire</button>
        <button type="button" className={tab === 'disponibilites' ? 'on' : ''} onClick={() => setTab('disponibilites')}>Disponibilités</button>
      </div>
        ...
        <>
          <div className="row" style={{ marginBottom: '1.1rem' }}>
            <div className="row" style={{ gap: '0.3rem' }}>
              ...
              <strong style={{ minWidth: 150, textAlign: 'center', textTransform: 'capitalize' }}>{dayShort(day)}</strong>
          ...
          <div className="kpis" style={{ marginBottom: '1.4rem' }}>
            <Kpi ic={Users} label="Collaborateurs" value={activePeople.length} sub="Équipe active" hero />
          ...
          {availRows.length === 0 && <div className="card card-pad muted">Aucune personne pour ce filtre.</div>}
            ...
            {availRows.map((p) => {
              ...
              return (
                <div key={p.id} className="avail-card">
                  ...
                  <div className="avail-card-name">{name(p)}</div>
                  <div className="avail-card-role">{specialtyLabel(p)}</div>
                  <div className="avail-card-next">
                    <div className="label">Prochaine affectation</div>
                    {next ? (
                      <>
                        <div className="ws"><span className="mono">{next.worksite.ref}</span> · {next.worksite.city ?? next.worksite.title}</div>
                        <div className="when">{toDateInput(new Date(next.startAt)) === day ? 'Aujourd’hui' : dayShort(toDateInput(new Date(next.startAt)))} · {hhmm(next.startAt)} – {hhmm(next.endAt)}</div>
                      </>
                    ) : <div className="when muted">Aucune affectation prévue</div>}
                  </div>
